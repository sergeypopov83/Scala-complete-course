...
class SQLAPI(resource: String) {
  ...
  /*logParameter[String](resource)
  val connect = connection(resource)
  logParameter[String](sql)
  openConnection(connect)
  val res = connect.execute(sql)
  logParameter[String](res)*/

  // def execute(sql: String): String = logParameter(openConnection((logParameter[String] _).andThen(connection)(resource)).compose(logParameter[String] _).andThen(execute)(sql))

  def execute(sql: String): String = logParameter[String]((logParameter[String] _).andThen(connection)(resource).execute(logParameter[String](sql)))
