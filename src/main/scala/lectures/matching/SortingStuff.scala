  ...
  def sortJunk(stuff: List[Stuff]): StuffBox = sort(stuff, StuffBox()) ///sort(stuff, StuffBox())
   ...
   // Поправьте логику метода
    ...
    private def putStuffInRightBox(item: Stuff, stuffBox: StuffBox): StuffBox = item match{
      case it@Watches(_, cost) if cost > 1000 => stuffBox.copy(watches = it :: stuffBox.watches)
      case it@Boots (brand, _) if brand == "Converse" || brand == "Adidas" => stuffBox.copy(boots = it :: stuffBox.boots)
